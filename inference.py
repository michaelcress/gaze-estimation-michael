...
def pre_process(image):
    ...
    # --- inside pre_process(image) before cvtColor ---
    if image is None or getattr(image, "size", 0) == 0:
        raise ValueError("Empty image passed to pre_process (read failed or empty ROI).")
...
def main(params):
    ...
    try:
        ...
        print("MODEL HEADS:", gaze_detector.fc_yaw.weight.shape, gaze_detector.fc_pitch.weight.shape)
